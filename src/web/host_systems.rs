...
use crate::db::{host_systems::*, models::*, DbExecutor};
use actix::prelude::*;
...
pub fn configure(cfg: &mut actix_web::web::ServiceConfig) {
    cfg.service(
        actix_web::web::scope("/host_systems")
...
fn all(
    ...
    db.send(Query::All(filters))
        ...
        .and_then(move |res| match res {
            ...
            Err(e) => {
                error!("Could not complete host_systems query: {:?}", e);
...
fn new(
    item: actix_web::web::Json<NewHostSystem>,
    db: actix_web::web::Data<Addr<DbExecutor>>,
    metrics: actix_web::web::Data<Metrics>,
) -> impl Future<Item = HttpResponse, Error = actix_web::Error> {
    metrics.inc_req_count();
    db.send(item.into_inner())
        ...
        .and_then(move |res| match res {
            Ok(r) => Ok(HttpResponse::Ok().json(json!(r))),
            Err(e) => {
                error!("Could not create New HostSystem: {:?}", e);
